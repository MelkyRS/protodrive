...
describe("Proto Drive API", () => {
  ...
  test("5. Download Chunk Metadata (New Route)", async () => {
    ...
    const res = await fetch(
      `${API_ROOT}/download/file/${testFile.id}/chunk/0`,
      { headers: AUTH_HEADER },
    );
  ...
  test("6. Search for the File", async () => {
    const res = await fetch(`${API_ROOT}/files/search?q=test-auto`, {
      headers: AUTH_HEADER,
    });
    const json = await res.json();
    expect(res.status).toBe(200);
  ...
  test("7. List Files with Pagination", async () => {
    const res = await fetch(`${API_ROOT}/files?limit=1&offset=0`, {
      headers: AUTH_HEADER,
    });
    const json = await res.json();
    expect(res.status).toBe(200);
  ...
  test("8. Get System Stats", async () => {
    const res = await fetch(`${API_ROOT}/system/stats`, {
      headers: AUTH_HEADER,
    });
    const json = await res.json();
    expect(res.status).toBe(200);
