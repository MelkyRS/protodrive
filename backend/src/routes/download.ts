 ...
 */
download.get("/chunk/:id", async (c) => {
  const id = c.req.param("id");
  const idx = parseInt(c.req.query("idx") || "0");
  ...
  if (
    !cdnUrl ||
    (cdnUrl.includes("ex=") &&
      parseInt(new URL(cdnUrl).searchParams.get("ex") || "0", 16) <
        Date.now() / 1000)
  ) {
    try {
      ...
      logger.error(
        ...
        `Failed to refresh URL for chunk ${idx}:`,
      );
    }
  }
...
download.get("/:id", async (c) => {
  ...
  try {
    ...
    if (chunksError) return c.text("Failed to fetch chunks", 500);
    ...
    if (chunkIndexStr !== undefined) {
      ...
      if (!response.ok)
        return c.text(`Discord fetch failed: ${response.status}`, 502);
    ...
    (async () => {
      try {
          ...
          while (attempt < MAX_ATTEMPTS) {
            ...
            if (isExpired || (isRetry && chunk.message_id)) {
              try {
                ...
                if (isRetry)
                  logger.debug(
                    `[UPSTREAM RETRY] Refreshing URL for chunk ${chunk.idx}`,
                  );
                ...
                logger.warn(
                  e instanceof Error ? e : new Error(String(e)),
                  `Stream JIT refresh warning for chunk ${chunk.idx}:`,
                );
              }
            }
            ...
            try {
              ...
              logger.debug(
                `[UPSTREAM START] Chunk ${chunk.idx + 1}/${chunks.length} (Attempt ${attempt})`,
              );
              const response = await fetch(cdnUrl!, {
                signal: AbortSignal.timeout(120000),
              }); // Increased to 120s for slow connections
              ...
              logger.debug(
                `[UPSTREAM DONE] Chunk ${chunk.idx + 1}/${chunks.length}`,
              );
              return buffer;
            } catch (err: unknown) {
              ...
              if (attempt < MAX_ATTEMPTS) {
                logger.warn(
                  `[UPSTREAM ERROR] Chunk ${chunk.idx} failed (Attempt ${attempt}): ${error.message}. Retrying...`,
                );
        ...
        const WINDOW_SIZE = 2;
        const chunkPromises: Array<Promise<ArrayBuffer> | null> = new Array(
        ...
        for (let i = 0; i < filteredChunks.length; i++) {
          ...
          if (lookAheadIndex < filteredChunks.length) {
            chunkPromises[lookAheadIndex] = fetchChunkData(
              filteredChunks[lookAheadIndex],
            );
          }
        ...
        if (errorMessage === "Client aborted") {
          ...
          logger.error(
            ...
            `[STREAM CRITICAL] File ${fileId} failed:`,
          );
        }
    ...
    logger.error(error as Error, "Download Error");
