...
stream.get("/file/:id", async (c) => {
  ...
  try {
    ...
    if (chunksError || !chunks) {
      return c.text("Problm fetching chunks", 500);
    }
    ...
    if (isExpired(cdnUrl) && targetChunk.message_id) {
      logger.debug(
        `Refreshing URL for streaming ${fileId} chunk ${targetChunk.idx}`,
      );
      try {
        // 1. Try Bulk Refresh
        if (cdnUrl) {
        ...
        await db
          .from("chunks")
          .update({ url: cdnUrl })
          .eq("message_id", targetChunk.message_id);
      } catch (e) {
        logger.warn(
            ...
            if (cdnUrl) {
              if (cdnUrl) {
                await db
                  ...
                  .eq("message_id", targetChunk.message_id);
                logger.info(
                  `Successfully recovered chunk ${targetChunk.idx} from backup channel`,
                );
              }
            }
          }
    ...
    logger.error(error, `[STREAM CRITICAL] Error for ${fileId}:`);
