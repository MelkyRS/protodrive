...
export function Sidebar({
  ...
  const menuItems = [
    { icon: Files, label: "My Drive", value: "active" as const },
  ...
  const usagePercent = stats
    ? (stats.storage.totalSize / (100 * 1024 * 1024 * 1024)) * 100
    : 0; // Soft limit visualization
  ...
  const handleUploadClick = () => {
    ...
    setTimeout(() => {
      ...
      onOpenUpload?.(); // Call the optional prop function
  ...
  return (
    <div
      className={cn(
        "flex flex-col h-full bg-card/50 backdrop-blur-xl border-r border-border/40 p-5",
        className,
      )}
    >
        ...
        >
          ...
          <div className="flex flex-col items-start -space-y-0.5">
            <span className="text-sm font-bold tracking-tight">
              {isProcessing ? "Opening..." : "New Files"}
            </span>
          </div>
        </Button>
      </div>
          ...
          <div className="space-y-1">
            {menuItems.map((item, i) => (
              ...
              >
                <Button
                  variant={
                    currentView === item.value && !item.disabled
                      ? "secondary"
                      : "ghost"
                  }
          ...
          <div className="relative z-10 space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span
                className={cn(
                  "text-[10px] font-bold",
                  usagePercent > 90 ? "text-red-500" : "text-primary",
                )}
              >
            ...
            <div className="space-y-2">
              ...
              <Progress
                value={
                  stats && stats.storage.totalSize > 0
