...
export function TransferProvider({ children }: { children: ReactNode }) {
  ...
  const [remoteDownload, setRemoteDownload] = useState<ReturnType<
    typeof useDownload
  > | null>(null);
  const [remoteUpload, setRemoteUpload] = useState<ReturnType<
    typeof useUpload
  > | null>(null);

  useEffect(() => {
    const channel = new BroadcastChannel("proto-transfers");
  ...
  useEffect(() => {
    ...
    const activeDownload = download.isDownloading
      ? download
      : remoteDownload?.isDownloading
        ? remoteDownload
        : null;
    const activeUpload = upload.isUploading
      ? upload
      : remoteUpload?.isUploading
        ? remoteUpload
        : null;
    ...
    const showNotification = (title: string, body: string) => {
      ...
      if (Notification.permission === "granted") {
        new Notification(title, { body, icon: "/proto.svg" });
  ...
  const exposedDownload = download.isDownloading
    ? download
  ...
  return (
    <TransferContext.Provider
      value={{
        upload: exposedUpload,
        download: exposedDownload,
        isUploadWidgetOpen,
