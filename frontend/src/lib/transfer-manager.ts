...
// Configuration
const CONCURRENCY = 3;
const CHUNK_SIZE = 4096 * 1024; // 4MB (Vercel limit is 4.5MB)
...
function initWorker(
  ...
  return new Promise((resolve, reject) => {
    ...
    worker.postMessage({
      type: "INIT",
      payload: { password: masterKey, salt },
    });
  });
}
...
export async function processDownload({
  ...
  try {
    ...
    const responseMetadata = await api.get<
      FileMetadata & { chunks: ChunkMetadata[] }
    >(`/files/${fileId}`);
    ...
    const totalBytes =
      responseMetadata.size || chunks.reduce((acc, c) => acc + c.size, 0);
    let downloadedBytes = initialBlobs.reduce(
      (acc, b) => acc + (b?.size || 0),
      0,
    );
    let sessionBytes = 0;
    ...
    const createChunkTask =
      (_chunk: ChunkMetadata, index: number) => async (): Promise<Blob> => {
        ...
        while (retries < 3) {
          try {
            ...
            const arrayBuffer = new Uint8Array(
              chunksArr.reduce((a, b) => a + b.length, 0),
            );
            let offset = 0;
            ...
            worker.postMessage(
              {
                type: "DECRYPT_CHUNK",
                payload: { chunk: finalBuffer, index, iv: fileIv },
              },
              [finalBuffer],
            );
            finalBuffer = await new Promise((resolve, reject) => {
              const h = (e: MessageEvent) => {
                if (
                  e.data.type === "CHUNK_DECRYPTED" &&
                  e.data.payload.index === index
                ) {
    ...
    const finalBlob = new Blob(blobs, {
      type: responseMetadata.type || "application/octet-stream",
    });
...
export async function processUpload({
  ...
  try {
    ...
    const data = encoder.encode(
      `${file.name}-${file.size}-${file.lastModified}-${CHUNK_SIZE}`,
    );
    ...
    const createUploadTask = (index: number) => async (): Promise<void> => {
      ...
      if (shouldEncrypt) {
        worker!.postMessage(
          { type: "ENCRYPT_CHUNK", payload: { chunk: ab, index, iv } },
          [ab],
        );
        payload = await new Promise((resolve, reject) => {
          const h = (e: MessageEvent) => {
            if (
              e.data.type === "CHUNK_ENCRYPTED" &&
              e.data.payload.index === index
            ) {
              worker!.removeEventListener("message", h);
              resolve(e.data.payload.chunk);
        ...
        xhr.upload.onprogress = (e) => {
          if (e.lengthComputable && onProgress) {
            ...
            if (now - lastUpdate > 200) {
              ...
              const prog = Math.min(
                99,
                Math.round(sendProg * 30 + confirmProg * 70),
              );
              const elapsed = (now - sessionStartTime) / 1000;
